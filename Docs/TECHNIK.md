...
- `APokeMonsterBattleTestController` startet die Testbegegnung, erstellt das Widget, aktiviert Maus/UI-Eingabe und löst die gewählte Runde nach 0,35 Sekunden aus. Der Presenter sperrt weitere Eingaben sofort bei Auswahl und bis `FinishPresentation`. Doppelte Auflösung und vorzeitiges Entsperren werden abgewiesen. Spätere Animationen können denselben Abschluss verwenden.
- `UPokeMonsterBattleWidget` ist ein natives UMG-Widget mit skalierter Oberfläche, HP-Balken, vier Buttons, scrollendem Log und zwei aus einfachen UMG-Formen aufgebauten Kreaturen-Platzhaltern. Die eigene Kreatur steht unten links, der Gegner oben rechts. K.O. wird durch Text und reduzierte Deckkraft angezeigt. Die Gestaltung kann später durch eine Widget-Blueprint-Unterklasse mit den gleichnamigen Controls und den Ereignissen `OnBattleViewUpdated`/`OnBattleRoundResolved` ersetzt oder ergänzt werden; `WidgetClass` am Testcontroller ist dafür konfigurierbar.
...
Für die Oberfläche kommen die Engine-Module UMG, Slate und SlateCore hinzu, keine externen Plugins oder Grafik-Assets. Die Map ist das einzige neue Content-Asset; es wird kein Widget-Blueprint benötigt. Die Platzhalter sind ausdrücklich Testgrafiken und ersetzen nicht die beschlossene visuelle Stilrichtung.

`PokeMonster.Battle.UI.BindingAndInputLock` und `PokeMonster.Battle.UI.UnavailableMoves` prüfen die Datenanbindung, Eingabesicherung, PP/HP/Log, Status-Platzhalter, K.O./Ende, Neustart sowie fehlende Daten und nicht verfügbare Attacken. Zum manuellen Test die Battle-Testmap öffnen und Play starten.
