...
`UPokeMonsterInventorySubsystem` lebt in der Game Instance und hält individuelle Stapel als weiche Item-Referenz plus Menge. `AddItem` verteilt Mengen auf begrenzte Stapel, `RemoveItem` prüft den Gesamtbestand vor jeder Änderung, und `GetQuantity`/`HasItem` arbeiten stapelübergreifend. `GetStacks` und `RestoreStacks` bilden die validierte Übergabe an das Save-System; eine Inventar-UI gibt es noch nicht. Zum Test erhält jede neue Game Instance einmalig fünf `DA_TestCaptureItem`; ein erfolgreich geladener Spielstand ersetzt diesen Testbestand vollständig. Kämpfe füllen den Bestand nicht nach.
...
`Save/PokeMonsterSaveGame` ist ein versioniertes `USaveGame` (Schema 1), `UPokeMonsterSaveSubsystem` ein `UGameInstanceSubsystem`. Der einzige reguläre Slot heißt `PokeMonster_Dev`. `SaveCurrentGame`, `LoadGame`, `HasSaveGame` und `DeleteDevSave` sind Blueprint-aufrufbar; in PIE stehen zusätzlich die Konsolenbefehle `PMSave`, `PMLoad`, `PMHasSave`, `PMDeleteDevSave` und `PMDevSaveRoundTrip` am Player zur Verfügung. Der Round-Trip-Test verweigert das Überschreiben eines bereits vorhandenen Dev-Saves. Es gibt noch kein Menü, Autosave, Cloud-Sync oder weitere Spielstände. Der Slot liegt unter Unreals ignoriertem Verzeichnis `Saved/SaveGames` und gehört nicht in Git.

Der Snapshot speichert Teammitglieder mit Instanz-ID, Spezies-ID, Level, kumulativer XP, aktuellen HP sowie vier Attacken-IDs und aktuellen PP. Inventarstapel enthalten Item-ID und Menge. Besiegte Trainer-IDs und abgeschlossene Encounter-IDs halten den derzeit relevanten Weltzustand fest: der sichtbare Wildactor nach Sieg/Fang und die einmalig ausgelöste Testzone. Diese Flags verwenden konfigurierbare stabile Encounter-IDs. Spezies-, Attacken- und Itemdefinitionen bleiben ausschließlich in den Primary Data Assets; berechnete Statuswerte und Max-PP werden beim Laden aus den Assets neu ermittelt.

Das Laden löst jede Primary Asset ID über den Asset Manager auf und prüft Typ, Existenz und Identität, bevor Runtime-Systeme geändert werden. Level/XP-Konsistenz, HP-Grenzen, Moveslots/PP, Teamgröße, doppelte Instanz-IDs, Stapelgrenzen und Schema-Version werden ebenfalls geprüft. Fehlende oder ungültige Daten werden geloggt und der bisherige Runtime-Zustand bleibt erhalten. Das Schema kennt derzeit nur Version 1; für künftige Versionen ist die Versionsprüfung der zentrale Migrationspunkt, ein unbekanntes älteres oder neueres Format wird abgewiesen. Speichern und Laden sind während eines aktiven Encounter gesperrt. Automatisierte Tests liegen unter `PokeMonster.Save`; der PIE-Diagnosebefehl verändert Team, Inventar und Flags, speichert sie, leert den Zustand, lädt ihn und prüft HP, XP, PP, Bestände und Flags.
