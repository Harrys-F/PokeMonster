...
`UPokeMonsterInventorySubsystem` lebt in der Game Instance und hält individuelle Stapel als weiche Item-Referenz plus Menge. `AddItem` verteilt Mengen auf begrenzte Stapel, `RemoveItem` prüft den Gesamtbestand vor jeder Änderung, und `GetQuantity`/`HasItem` arbeiten stapelübergreifend. `GetStacks` und `RestoreStacks` erlauben später eine Savegame-Übergabe mit Validierung; ein tatsächliches Savegame und eine Inventar-UI gibt es noch nicht. Zum Test erhält jede neue Game Instance einmalig fünf `DA_TestCaptureItem`; Kämpfe füllen den Bestand nicht nach.
